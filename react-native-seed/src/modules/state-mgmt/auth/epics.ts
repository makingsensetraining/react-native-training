  ...
  action$.pipe(
    ...
    mergeMap(action => concat(
      ...
      of(action).pipe(
        ...
        switchMap((res: GeneralModel.ILoginResponse) => [
          actions.success(res._id),
          coreState.actions.bootstrap(res.access_token)
        ])
      ),
      of(actions.setLoading(false))
    ).pipe(
      catchError(() => of(actions.fail(), actions.setLoading(false)))
    ))
  );
